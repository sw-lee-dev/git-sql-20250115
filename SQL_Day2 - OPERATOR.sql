...
ALTER TABLE transaction CHANGE tex tax TEXT;
...
-- IS : 좌항이 우항과 같으면 true (키워드 기준으로 비교)
-- IS NOT : 좌항이 우항과 다르면 true (키워드 기준으로 비교)
SELECT * FROM transaction
...
-- NOT BETWEEN min AND mas : 좌항이 min보다 작거나 max보다 크면 true
SELECT * FROM transaction
...
-- AND 또는 && : 좌항과 우항이 모두 true면 true
SELECT * FROM transaction
WHERE transaction_type = '구매' AND amount > 100000;

-- OR 또는 || : 좌항과 우항 중 하나라도 true이면 true
SELECT * FROM transaction
...
-- 동일한 결과 >> WHERE transaction_date >= '2025-01-20' AND breakdown != '기타자재' OR transaction_date < '2025-01-20' AND breakdown = '기타자재';

-- NOT 또는 ! : 부정 연산자, 우항이 true면 false, false면 true
SELECT * FROM transaction
