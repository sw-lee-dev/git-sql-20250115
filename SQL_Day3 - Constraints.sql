...
-- [자기 자신 테이블의 INSERT 또는 UPDATE 작업에 영향을 미침]
CREATE TABLE not_null_table (
	null_column INT NULL,
    not_null_column INT NOT NULL
);
-- NOT NULL 제약조건이 지정된 컬럼을 삽입시 선택하지 않았을 경우
-- Error Code: 1364, Feild 'not_null_column' doesn't have a default value.
...
-- NOT NULL 제약조건이 지정된 컬럼에 null로 수정할 경우
-- Error Code: 1048, Column 'not_null_column' cannot be null.
...
-- UNIQUE 제약조건이 지정된 컬럼에 중복된 데이터를 지정할 경우
-- Error Code: 1062, Duplicate entry '1' for key 'unique_table.unique_column'.
INSERT INTO unique_table VALUES (1, 1);
...
-- 제약조건 지정 방법
-- [CONSTRAINT 제약조건의이름]=생략가능 제약조건 (선택할 컬럼)
CREATE TABLE composite_table (
	pk1 INT,
    pk2 INT,
    CONSTRAINT composite_table_pk PRIMARY KEY (pk1, pk2)
    -- PRIMARY KEY (pk1, pk2) 도 가능
);
...
ALTER TABLE not_null_table MODIFY COLUMN null_column INT NOT NULL; -- null_column에 null값이 있는 경우는 Error code: 1138, Invalid use of NULL value. 에러가 발생
ALTER TABLE not_null_table ADD CONSTRAINT not_null_table_uq UNIQUE (null_column); -- null_column 에 중복된 값이 있어서 에러 발생

-- FOREIGN KEY 제약조건 : 특정 컬럼을 다른 테이블 혹은 같은 테이블의 기본키 컬럼과 연결하는 제약
-- 주의. FOREIGN KEY 제약조건이 지정되는 컬럼은 참조하고자 하는 컬럼의 데이터타입과 일치해야함
...
-- ON UPDATE / ON DELETE 옵션(FOREIGN KEY의)
-- ON UPDATE : 부모 테이블의 기본키가 변경될 때 동작
-- ON DELETE : 부모 테이블의 기본키가 삭제될 때 동작

-- RESTRICT : 부모 테이블의 기본키의 수정 및 삭제를 불가능하게 함 (기본값)
-- CASCADE : 부모 테이블의 기본키가 수정 또는 삭제된다면, 자식 테이블의 외래키도 수정 또는 삭제됨
-- SET NULL : 부모 테이블의 기본키가 수정 또는 삭제된다면, 자식 테이블의 외래키는 null로 지정
...
-- Error Code: 3819, Check constraint 'check_table_chk_1' os violated.
...
-- [자기 자신 테이블의 INSERT 작업에만 영향을 미침]
CREATE TABLE default_table (
	-- AUTO_INCREMENT : 기본키에서 데이터타입이 정수형일때 값을 1씩 증가하는 값으로 자동 지정
