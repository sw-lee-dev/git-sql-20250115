...
ALTER TABLE employee ADD CONSTRAINT employee_department_code_fk FOREIGN KEY (department_code) REFERENCES department (department_code);
...
-- 컬럼 및 테이블에서 사용 가능
-- 결과 혹은 원래 이름을 다르게 지정하고 싶을때 사용
SELECT employee_number AS '사번', name AS '사원이름', age AS '사원나이', department_code AS '부서코드'
FROM employee AS e;

-- AS 키워드는 생략 가능
SELECT employee_number '사번', name '사원이름', age '사원나이', department_code '부서코드'
FROM employee e;
...
SELECT 
    ...
    E.name '사원이름', E.age '사원나이',
    E.department_code '부서코드',
    D.name '부서이름', D.tel_number '부서 전화번호'
...
-- My SQL에서는 FULL OUTER JOIN을 문법상으로 지원하지 않음
...
-- My SQL에서 기본 JOIN이 CROSS JOIN 임
SELECT * FROM employee E JOIN department D;
-- JOIN 생략 가능
SELECT * FROM employee E, department D;

-- 부서코드가 A 인 사원에 대해 사번, 사원 이름, 부서명을 조회하시오.
SELECT 
