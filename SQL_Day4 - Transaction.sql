...
-- 트랜잭션 (Transaction) : SQL의 작업 단위를 하나로 묶어서 처리하는 기능
-- 데이터 무결성을 보장, 잘못된 데이터 처리가 발생했을때 롤백을 가능하게 해줌

-- ACID 특성
-- Atomicity (원자성) : 하나의 트랜잭션은 모두 성공하거나 모두 실패해야함
-- Consistency (일관성) : 트랜잭션이 완료되면 데이터베이스의 상태가 일관성을 유지해야함
...
-- COMMIT : 트랜잭션의 변경 사항을 데이터베이스에 영구히 반영 (트랜잭션의 성공적 완료을 뜻함)
-- ROLLBACK : 트랜잭션의 작업 내용을 이전 상태로 복원
-- SAVEPOINT : 트랜잭션 내부에서 ROLLBACK 시 돌아올 위치 지정
-- SET AUTOCOMMIT : 자동 커밋 (1 - 활성화, 0 - 비활성화)

CREATE USER 't_user'@'localhost' IDENTIFIED BY 'qwer1234';

GRANT ALL ON practice_sql.* TO 't_user'@'localhost';
