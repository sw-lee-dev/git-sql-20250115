...
-- 하나의 수업은 하나의 교실에서 이루어지고 한명의 교사에 의해서 강의되어지며, 여러명의 학생이 수업을 들을 수 있음
-- 단, 수업은 반드시 교실 및 교사가 존재해야함
-- 교실은 여러 수업을 진행할 수 있고, 교사는 여러 수업을 강의할 수 있음
-- 학생은 여러 수업을 수강할 수 있음

-- 교실 1 : n 수업 => n의 관계인 수업이 교실에 종속
-- 교사 1 : n 수업 => n의 관계인 수업이 교사에 종속
-- 학생 n : m 수업 => 관계테이블 수강 생성
...
CREATE TABLE teacher (
    ...
    position VARCHAR(5) NOT NULL DEFAULT '일반교사',
...
CREATE TABLE class (
    ...
    CONSTRAINT class_class_room_fk FOREIGN KEY (class_room) REFERENCES class_room (class_room_number),
    CONSTRAINT class_teacher_fk FOREIGN KEY (charge_teacher) REFERENCES teacher (teacher_number)
);
...
CREATE TABLE class_regist (
    ...
    CONSTRAINT class_regist_student_fk FOREIGN KEY (student_number) REFERENCES student (student_number),
    CONSTRAINT class_regist_class_fk FOREIGN KEY (class_code) REFERENCES class (class_code)
);
...
-- 2. MT1 수업을 수강하는 학생의 이름, 주소, 휴대전화번호, 점수를 조회하는 쿼리문을 작성하시오.
SELECT 
	S.name '학생의 이름',
    S.address '학생의 주소',
    S.tel_number '학생의 휴대전화번호',
    CR.score '학생의 점수'
...
-- 7. 평균 점수(수학 + 국어 + 영어)가 90점 이상인 학생이 수업중인 교실의 층 및 좌석수를 조회하는 쿼리문을 작성하시오.
SELECT
	floor '층',
    seats '좌석수'
FROM class_room
WHERE class_room_number IN (
	SELECT DISTINCT class_room -- 중복없이 가져온 수업에 해당하는 교실번호를 가져옴
	FROM class
	WHERE class_code IN (
		SELECT DISTINCT class_code -- 수강 테이블에서 평균 점수가 90점 이상인 학번의 학생들과 같은 학번의 학생들이 듣는 수업을 중복 제거하고 가져옴
		FROM class_regist
		WHERE student_number IN(
			SELECT student_number -- 평균 점수가 90점 이상인 학번만 아래 서브쿼리로 가져옴
			FROM (
				SELECT -- 평균 점수가 90점 이상인 학생들의 학번과 평균점수를 가져옴
					student_number,
					AVG(score) 'avg'
				FROM class_regist
				GROUP BY student_number
				HAVING avg >= 90
			) SUB -- FROM 절에서 서브쿼리 후 Alias 필수
		)
	)
);